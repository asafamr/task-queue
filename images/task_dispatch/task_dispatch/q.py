...
def http_task(task_id: str, task_type: str, params: dict):
    ...
    task_url = os.getenv(
        "HTTP_TASK_URL_TEMPLATE",
        "http://task-svc-{task_type}.default.svc.cluster.local/run",
...
def sync_db_and_queue():
    with try_global_lock("DB_SYNC", ttl_sec=10 * 60) as got_lock:
        ...
        for batch in gen_batches(current_status.items(), batch_size):
            ...
            req = req_session.patch(
                manager_url + "/v1/internal/tasks/work", json=work_done, timeout=30
            )
            req.raise_for_status()
            remove_tasks(
                [
                    uid
                    for uid, status in current_status.items()
        ...
        while True:
            ...
            req = req_session.get(
                manager_url + "/v1/internal/tasks/work", params=params, timeout=30
            )
            req.raise_for_status()
            ...
            deleted_tasks = [
                x for x in current_status if x not in all_running_tasks_uids
            ]
