...
async def get_db_client():
    ...
    await mongo_client.tasks.tasks.create_index([('finished', pymongo.ASCENDING)], 
        partialFilterExpression = { 'finished': { '$eq': False } } , background = True)
...
class DbClient:
    ...
    async def get_user_task(self, user_id:str, task_id:str) -> Optional[Task]: 
        collection:AgnosticCollection = self.db.tasks
        ret =  await collection.find_one({'user_id':{ '$eq': user_id }, '_id':{ '$eq': ObjectId(task_id) }}, )
        if not ret:
            return None
    ...
    async def get_workable_batch(self, batch_size = 100, start_uid=None,stale_task_after_sec=5*60,) -> str:
        collection:AgnosticCollection = self.db.tasks
        ...
        cursor = collection.find({'$and':filters
            }).limit(batch_size)
        return [Task.from_db_dict(x) async for x in cursor]
    ...
    async def update_work_batch(self, work_done: List[TaskStatusUpdate]) -> str:
        ...
        res = await collection.bulk_write([
            UpdateOne({'_id': ObjectId(x.uid)}, {'$set':{'status': x.status, 
                # 'last_update':now_date, 
                'finished': x.status in [TaskStatus.FAILED,TaskStatus.SUCCEEDED],
                'additional_info':x.additional_info}}) for x in work_done
        ], ordered=False)
