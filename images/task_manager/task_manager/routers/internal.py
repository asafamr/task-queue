...
async def get_tasks_work_batch(request: Request, batch_size:int=100,start_uid:Optional[str]=None) -> List[Task]:
...
async def update_tasks_work_batch(request: Request, work_done:List[TaskStatusUpdate]=Body()):
    dbc:DbClient = request.app.state.dbc
